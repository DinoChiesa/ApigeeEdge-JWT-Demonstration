...
### Verify a RS256 JWT, retrieving the public key from KVM
...
This loads the key into the KVM (non-encrypted), using the Apigee Edge management API. You
do not NEED to load the key using this tool. But you do need to insure
that the PEM-encoding for the key is unmodified. It should use a single
line feed (\n), rather than a CR-LF combination (\r\n), for each new line.
That node script does this automatically.
...
## Using Apigee Edge to Verify a JWT generated by Google Signin
...
## Using Apigee Edge to Generate a JWT
...
## Using Apigee Edge to Generate a JWT with additional claims
...
## Using Apigee Edge to Generate a JWT, retrieving the key from encrypted KVM
...
This loads the key into the encrypted KVM, using the Apigee Edge management API. You
do not NEED to load the key using this tool. But you do need to insure
that the PEM-encoding for the key is unmodified. It should use a single
line feed (\n), rather than a CR-LF combination (\r\n), for each new line.
That node script does this automatically.
...
## Note: The JWT Policies in Apigee Edge
